...
2. From the **Microsoft Fabric** resource page, click on **+ Create**.
...
3. Fill the following details as provided below:
   ...
   - **Capacity name**: **snowflakefabric<inject key="DeploymentID" enableCopy="false"/>** (2)
   - **Region**: **WEST US 2** (3)
...
5. Navigate back to the **Fabric portal** home page and select **Power BI** from the bottom pane.
...
9. Now the page will be redirected to the new workspace that is created, choose **+New item** from that workspace.

   ![](../media/Lab-04/newitem.png)

10. Search for **Lakehouse** in the **Filter for item type** bar and select it.
...
13. In the Properties Menu, copy the **URL** and paste it in notepad as it will be used in further steps.

    ![](../media/Lab-04/url.png)

14. You will need your Azure Tenant ID. To find this in the Fabric UI:
    - In the upper right click on the **?** for **help & support (1)** and at the bottom click **About Power BI (2)**.

      ![](../media/Lab-04/powerbi-1.png)

    - Select and copy the Tenant URL. The Tenant ID is the UUID that follows `ctid=` and paste it in the notepad.

      ![](../media/Lab-04/powerbi-01.png)

      > **NOTE**: Make sure to copy only the Tenant URL as highlighted below.
...
34. Return to the Snowflake session opened in the browser and run the following to create the Iceberg table and insert data from the sample dataset.
...
37. In order for Fabric to work on the Iceberg tables, you need to set up shortcuts to the data.
...
38. This will return a path to the metadata file for this table, which should show you which storage account contains the Iceberg table.
...
39. Navigate back to Fabric and open the **snowflakefabric-<inject key="DeploymentID" enableCopy="false"/>** workspace, which contains the **snowflakeQS** Fabric lakehouse object.

40. Click **Workspace settings**.

    ![](../media/Lab-04/workspace-settings.png)

41. Under **Delegated settings**, click **OneLake settings**, and turn on the **Authenticate with OneLake user-delegated SAS tokens** setting.
...
45. Select **snowflakeQS (1)** lakehouse and then click **Next (2)**.

    ![](../media/Lab-04/snowflake-1.png)

46. Expand **Files (1)**, select **dim_customer (2)**, and click **Next (3)**.

    ![](../media/Lab-04/dim-customer-1.png)

47. In the "New Shortcut" window, click **Create**.

    ![](../media/Lab-04/create.png)

48. You can see the newly created shortcut.
